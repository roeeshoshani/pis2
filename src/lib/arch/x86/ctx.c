...
static err_t do_movs(const post_prefixes_ctx_t* ctx, pis_operand_size_t operand_size) {
    ...
    pis_operand_t chunk_tmp = LIFT_CTX_NEW_TMP(ctx->lift_ctx, operand_size);
    LIFT_CTX_EMIT(ctx->lift_ctx, PIS_INSN2(PIS_OPCODE_LOAD, chunk_tmp, si));
    LIFT_CTX_EMIT(ctx->lift_ctx, PIS_INSN2(PIS_OPCODE_STORE, di, chunk_tmp));
...
static err_t do_stos(const post_prefixes_ctx_t* ctx, pis_operand_size_t operand_size) {
    err_t err = SUCCESS;
    ...
    pis_operand_t ax = operand_resize(&RSI, operand_size);
    pis_operand_t di = operand_resize(&RDI, ctx->addr_size);
...
static err_t
    ...
    } else if (first_opcode_byte == 0xaa) {
        // stosb
        CHECK_RETHROW(do_stos(ctx, PIS_OPERAND_SIZE_1));
    } else if (first_opcode_byte == 0xab) {
        // stos[w/d/q]
        CHECK_RETHROW(do_stos(ctx, ctx->operand_sizes.insn_default_not_64_bit));
    } else {
        CHECK_FAIL_TRACE_CODE(
            PIS_ERR_UNSUPPORTED_INSN,
