...
static err_t post_prefixes_lift(const post_prefixes_ctx_t* ctx) {
    ...
    if ((first_opcode_byte & (~0b111)) == 0x50) {
        ...
        LIFT_CTX_ADVANCE(ctx->lift_ctx);
        modrm_t modrm = decode_modrm_byte(LIFT_CTX_CUR(ctx->lift_ctx));
        pis_operand_size_t operand_size = get_effective_operand_size(ctx, false);
        if (modrm.mod == 0b11) {
